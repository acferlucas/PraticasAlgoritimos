...
int mainL() {
	...
	try {
		cerr << "Testando adiciona() [normal]: " << endl;
		for (int i = 1; i <= MAX; i++) {
			lista->adiciona(i);
		}
		cerr << "OK" << endl;
	...
	for (int i = MAX; i >= 1; i--) {
		try {
			if (lista->pega(i) != i) {
				cerr << "FALHOU em " << i <<":"<< lista->pega(i) << endl;
				lista->exibe();
				exit(1);
			}
		} catch (const char * ex) {
			cerr << "FALHOU em " << i << " (exception)" << ex << endl;
			lista->exibe();
			exit(1);
		}
	}
	...
	cerr << "Testando remove() [pares]: "; // remove even elements of three forward
	try {
